...
const WA_ADMIN = '6281234567890' // ganti nomor WhatsApp admin RS
...
const Konfirmasi = () => {
  ...
  useEffect(() => {
    ...
    }, [dataPasien, dataPendaftaran, navigate]) // ← tambah navigate
  ...
  // Generate teks WhatsApp — sama persis dengan logika Laravel
  const generateWaText = () => {
    return `*[Pendaftaran Pasien]*
===========================
*No RM         :* ${dataPasien.no_rekam_medik ?? '-'}
*Nama          :* ${dataPasien.nama}
*NIK           :* ${dataPasien.nik}
*Telephone     :* ${dataPasien.no_telepon}
*Jenis Kelamin :* ${dataPasien.jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan'}
*Tanggal Lahir :* ${new Date(dataPasien.tanggal_lahir).toLocaleDateString('id-ID')}
*Tipe Pasien   :* ${dataPasien.tipe_pasien}
${dataPasien.no_bpjs ? `*No BPJS       :* ${dataPasien.no_bpjs}\n` : ''}*Alamat        :* ${dataPasien.alamat}

*Poliklinik    :* ${dataPendaftaran.poliklinik?.nm_poli ?? '-'}
*Dokter        :* ${dataPendaftaran.dokter?.nm_dokter ?? '-'}
*Tanggal Periksa:* ${formatTanggal(dataPendaftaran.tanggal_periksa)}
*Keluhan       :* ${dataPendaftaran.keluhan || '-'}`
  }

  const waUrl = `https://wa.me/${WA_ADMIN}?text=${encodeURIComponent(generateWaText())}`

  const handleSelesai = () => {
    reset() // bersihkan context
    navigate('/')
  }
        ...
        <div className="p-6 border-b border-gray-100">
          ...
          <dl className="grid grid-cols-2 gap-y-3 text-sm">
            ...
            <dd className="font-medium text-gray-800">
              {dataPasien.jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan'}
            </dd>

            <dt className="text-gray-400">Tanggal Lahir</dt>
            <dd className="font-medium text-gray-800">
              {new Date(dataPasien.tanggal_lahir).toLocaleDateString('id-ID', {
